...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const createScene = function () {
        ...
        /*for (const rope of ropes) {
            if (i == 4) { i = 0; }
            if (i == 0 && rope.material == blueCornerMat) {
                rope.material = redCornerMat;
            }
            else if (i == 0 && rope.material == redCornerMat) {
                rope.material = blueCornerMat;
            }
        ...
        const baseDistanceFromRing = 6; // Base distance from the edge of the ring to the first row of stands
        const distanceIncrement = 1; // Additional distance each higher row is from the ring compared to the one below
        const standIncrement = 1; // Distance to move the stands back for each row to ensure visibility
        ...
        for (let i = 0; i < numberOfRows; i++) {
            const yPosition = standHeight / 2 + (i * (standHeight + standSpacing));
            ...
            for (let j = -2; j <= 2; j++) {
                ...
                const xPosition = j * (standWidth + standSpacing); // Calculate the x position for each stand
